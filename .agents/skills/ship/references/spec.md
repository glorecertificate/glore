...
- Must match the official template at `.agents/skills/ship/assets/certificate-template.pdf`: Inter font, teal `#0f766e`, GloRe header/logo, QR code linking to `/{username}?v={handle}`, reviewer signature block.
...
**Current provider:** OpenAI (via Vercel AI SDK `@ai-sdk/openai`).
...
4. Should Gemini replace OpenAI entirely or serve as a fallback?
5. What R2 bucket policy for public vs private files? Certificate PDFs must be public.
6. Should the docs section support user comments or just admin-authored content?
7. Should there be an in-app notification system beyond email? (Currently email only.)
8. Should learners have any path to certificate eligibility, or is it strictly volunteers only?
