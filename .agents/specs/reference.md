...
**List exit/enter animations:** `AnimatedList` + `AnimatedListItem` (`src/components/ui/animated-list.tsx`) wrap any removable list so items fade/scale out and survivors reflow smoothly instead of snapping. `AnimatedList` is a preset over `<AnimatePresence mode="popLayout" initial={false}>`; place it inside the list container (grid, `space-y`, `divide-y`, `ul`) and map items to `AnimatedListItem`. Put the React `key` on `AnimatedListItem` (it is the `AnimatePresence` child). Props: `variant="card"` (grids, scale exit) or `variant="row"` (lists), `asChild` to animate a semantic element (e.g. `<li>`); `useReducedMotion` is handled internally. For dnd-kit sortable lists the item node cannot be animated by motion (dnd-kit owns its `transform`): use `<AnimatedListItem exitOnly>` (motion animates only the exit, after the item has left dnd's control) and let dnd-kit's own FLIP fill the gap by passing `animateLayoutChanges={animateLayoutChangesAlways}` to `SortableItem` and `measuring={measureAlways}` to `Sortable` (both exported from `src/components/ui/sortable.tsx`; gate `animateLayoutChanges` on `useReducedMotion` since dnd-kit's FLIP ignores it). Reference usage: `src/components/features/courses/course-list/index.tsx`.
