...
> **MANDATORY:** Agents MUST use the `tmp/` folder in the repository root for ALL temporary operations (test files, scratch scripts, generated output, etc.). This folder is git-ignored.
...
3. Clean up temporary files when they are no longer needed
4. Never commit contents of `tmp/` to git

```bash
# ✅ Correct
echo "test" > tmp/test-output.txt
node tmp/test-script.ts
