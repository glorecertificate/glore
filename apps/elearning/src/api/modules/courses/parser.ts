...
export const parseCourse = createParser<'courses', typeof courseQuery, Course>(course => {
  ...
  return { ...rest, skill, lessons, enrolled, progress, completed, creator, lessonsCount, lessonsCompleted, status }
})

export const parseSkill = createParser<'skills', typeof skillQuery, Skill>(({ area, user_assessments, ...skill }) => ({
  ...skill,
  area: area ?? undefined,
