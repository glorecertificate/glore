...
import { i18n, type Translator } from '@/lib/i18n'
...
export default ({ error }: { error: AnyError }) => {
  ...
  const [t, setT] = useState<Translator<'Layout'> | undefined>(undefined)
  const locale = cookies.get('NEXT_LOCALE', { prefix: false }) ?? i18n.defaultLocale

  useEffect(() => {
    void (async () => {
      const messages = (await import(`../../config/translations/${locale}.json`)).default
      setT(createTranslator({ locale, namespace: 'Layout', messages }))
    })()
  })
          ...
          <main className="flex min-h-screen flex-col items-center bg-background px-4 py-12 text-center">
            <div className="relative flex w-full grow flex-col items-center justify-center gap-6">
              ...
              <div className="text-center">
                <h2 className="mb-4 font-bold font-mono text-2xl text-foreground tracking-tight">{t('errorTitle')}</h2>
                <p className="mb-8 font-mono text-foreground/75">{t('errorMessage')}</p>
