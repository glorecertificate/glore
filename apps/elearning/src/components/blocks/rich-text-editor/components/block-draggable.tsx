...
import { KEYS, type TElement, isType } from 'platejs'
import {
  MemoizedChildren,
  type PlateEditor,
...
const Gutter = ({ children, className, ...props }: React.ComponentProps<'div'>) => {
  ...
  return (
    <div
      {...props}
      className={cn(
        'slate-gutterLeft group/gutter-left -translate-x-full absolute top-0 z-50 flex h-full cursor-text group-hover/container:opacity-100 sm:opacity-0',
...
const createDragPreviewElements = (
  ...
  const resolveElement = (node: TElement, index: number) => {
    ...
    if (lastDomNode) {
      ...
      const distance = (domNodeRect?.top ?? 0) - (lastDomNodeRect?.bottom ?? 0)
