          ...
          <div className="flex flex-col gap-2">
              ...
              <>
                {suggestionText2Array(suggestion.text!).map((text, index) => (
                  <div className="flex items-center gap-2" key={index}>
                    <span className="text-sm text-muted-foreground">{t('suggestions.delete')}</span>
            ...
            {suggestion.type === 'insert' && (
              <>
                {suggestionText2Array(suggestion.newText!).map((text, index) => (
                  <div className="flex items-center gap-2" key={index}>
                    <span className="text-sm text-muted-foreground">{t('suggestions.add')}</span>
                    <span className="text-sm" key={index}>
                      {text || 'line breaks'}
                    </span>
                  </div>
            ...
            {suggestion.type === 'update' && (
              <div className="flex items-center gap-2">
                <span className="text-sm text-muted-foreground">
                  {Object.keys(suggestion.properties as AnyRecord).map(key => (
                    <span key={key}>
                      {'Un'}
                      {key}
                    </span>
                  ))}
                  {Object.keys(suggestion.newProperties as AnyRecord).map(key => (
          ...
          dataList.forEach(data => {
            ...
            switch (data.type) {
              ...
              case 'update': {
                properties = {
                  ...properties,
                  ...(data.properties as AnyRecord),
                }

                newProperties = {
                  ...newProperties,
                  ...(data.newProperties as AnyRecord),
                }
