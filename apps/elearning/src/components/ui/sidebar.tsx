...
export const SidebarProvider = ({
  ...
  const contextValue = useMemo<SidebarContext>(
    ...
    [isMobile, label, open, openMobile, setOpen, state, toggleSidebar]
  )
...
export const SidebarRail = ({ className, ...props }: React.ComponentProps<'button'>) => {
  ...
  return (
    <button
      aria-label={label}
      className={cn(
        '-translate-x-1/2 group-data-[side=left]:-right-4 [[data-side=left][data-collapsible=offcanvas]_&]:-right-2 [[data-side=right][data-collapsible=offcanvas]_&]:-left-2 absolute inset-y-0 z-20 hidden w-4 in-data-[side=left]:cursor-w-resize in-data-[side=right]:cursor-e-resize transition-all ease-linear after:absolute after:inset-y-0 after:right-2 after:w-[1px] after:bg-sidebar-border hover:after:w-[2px] group-data-[side=right]:left-0 group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full sm:flex [[data-side=left][data-state=collapsed]_&]:cursor-e-resize [[data-side=right][data-state=collapsed]_&]:cursor-w-resize',
        className
      )}
      data-sidebar="rail"
