...
export const sendEmail = async <T extends EmailTemplate>(template: T, options: SendEmailOptions<T>) => {
  ...
  const t = await getTranslations(`Email.${template}`, { locale })
  ...
  const props: TemplateOptions<T> = { from: sender, locale, preview, subject, to, ...emailProps }
  ...
  return await client.sendAsync({ from: sender, to, subject, text, attachment })
}
