...
export const logEntities = <T>(entities: Partial<Record<PublicTable, T[]>>) => {
  ...
  if (list.length === 0) return log.error(`No ${String(entity)} created`)
  log.success(`Created ${list.length} ${pluralize(String(entity), list.length)}`)
}
