...
PRETTIER_CONFIG=@repo/prettier-config/sql
...
if [ -z "$SUPABASE_TOKEN" ] || [ -z "$SUPABASE_DB_URL" ]; then
  echo "Error: SUPABASE_TOKEN and SUPABASE_DB_URL must be set"
  exit 1
fi
...
format_sql() {
  prettier --write $@ --config $PRETTIER_CONFIG >/dev/null 2>&1
}
...
sync_schema() {
  if supabase db dump --schema $SCHEMA --file $SCHEMA_PATH --yes $@ >/dev/null 2>&1 && format_sql $SCHEMA_PATH; then
    echo "Finished database schema update."
  fi
}
...
sync_types() {
  ...
  if supabase gen types typescript --schema $SCHEMA "$arg" > $TYPES_PATH; then
    echo "Finished database types generation."
  fi
}
...
case $cmd in
  ...
  typegen)
    if [ ! -f "$TYPES_PATH" ]; then
      mkdir -p "$(dirname "$TYPES_PATH")" && touch "$TYPES_PATH"
    fi
    sync_types
    ;;
  *)
