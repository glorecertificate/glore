...
import { type EmailProps, type EmailTemplate } from '@glore/mailer/types'
import { deepMerge } from '@glore/utils/deep-merge'

export interface LayoutProps extends HtmlProps, EmailProps<EmailTemplate> {}

const Footer = ({ footer, f, t }: LayoutProps) => (
  <Container className="mt-6">
    <Section>
      <Row>
        <Column className="text-right">
          <Link href={process.env.APP_URL}>{t.common.app}</Link>
        </Column>
        <Column className="text-center">
          <Link href={metadata.website}>{t.common.homepage}</Link>
        </Column>
        <Column className="text-left">
          <Link href={`mailto:${metadata.email}`}>{t.common.contactUs}</Link>
        </Column>
      </Row>
    </Section>
    <Section className="mb-10 text-center text-gray-400">
      {footer && <Text className="mb-1 text-sm">{footer}</Text>}
      <Text className="mb-1 text-sm">{f(t.common.copyright, { year: new Date().getFullYear() })}</Text>
      <Text className="mt-0 text-sm">{t.common.address}</Text>
    </Section>
  </Container>
)
  ...
  <Html lang={locale} title={subject} {...props}>
    ...
    >
      ...
      <Body className="bg-offwhite font-sans text-base">
        ...
        <Footer footer={footer} t={t} {...props} />
      </Body>
    </Tailwind>
  </Html>
)
