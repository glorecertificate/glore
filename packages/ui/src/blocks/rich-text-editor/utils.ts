...
import { isHotkey as isHotkeyBase, KEYS, PathApi, type NodeEntry, type Path, type TElement } from 'platejs'
import { type HotkeysOptions, type PlateEditor } from 'platejs/react'

import { ACTION_THREE_COLUMNS } from './config'
...
export const setBlockType = (editor: PlateEditor, type: string, { at }: { at?: Path } = {}) =>
  editor.tf.withoutNormalizing(() => {
    ...
    editor.api.blocks({ mode: 'lowest' }).forEach(entry => setEntry(entry))
  })
