...
const ARGS = ['public', 'next'] as const
const PUBLIC_DTS = './types/public.generated.d.ts'
...
if (types.includes('public')) {
  try {
    if (stdout.isTTY) stdout.write('Generating public file types...')

    const content = `// AUTO-GENERATED by scripts/typegen.mts. Do not edit.
declare global {
...
export {}`
    ...
    writeFileSync(PUBLIC_DTS, content, 'utf-8')

    clearLine()
    if (!ci) console.info(`${green('✓')} Public types generated successfully`)
  } catch {
    clearLine()
    console.error(`${red('✗')} Failed to write ${PUBLIC_DTS}`)
    process.exit(1)
  }
}
