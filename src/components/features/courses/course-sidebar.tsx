...
const CourseSidebarItem = memo(
  ({
    ...
    return (
      <StepperItem className="group relative w-full" loading={user.canEdit ? false : undefined} step={step}>
        <StepperTrigger
          className={cn(
...
export const CourseSidebar = (props: React.ComponentProps<'div'>) => {
  const { addLesson, course, initialCourse, language, removeLesson, setLesson, setStep, step } = useCourse()
  const { user } = useSession()
  const t = useTranslations('Courses')
